...
// ─── PROMPT PARA PROPIEDAD ESPECÍFICA (match por ID exacto) ───────────────
// Se usa cuando el lead manda un link de la web propia
// Claude recibe SOLO esa propiedad, sin posibilidad de mezclar datos

export function buildSinglePropertyPrompt(prop, env) {
  const dir = (prop.direccion && !prop.direccion.includes('\n') && prop.direccion !== prop.titulo && prop.direccion.length < 80)
    ? prop.direccion : null
...
El lead te consultó por una propiedad específica. Acá están los datos EXACTOS de esa propiedad — usá ÚNICAMENTE estos datos, no inventes ni agregues nada:

\${ficha}

INSTRUCCIONES:
1. Saludá y presentá la ficha con este formato exacto:

🏠 *\${prop.titulo}*
\${dir ? \`📍 *Ubicación:* \${dir}\n\` : ''}💰 *Precio:* \${prop.precio}\${prop.operacion === 'Alquiler' ? '/mes' : ''}
\${prop.supCubierta || prop.supTotal ? \`📐 *Superficie:* \${prop.supCubierta || ''}\${prop.supCubierta && prop.supTotal ? ' cubiertos / ' : ''}\${prop.supTotal || ''} totales\n\` : ''}\${prop.ambientes ? \`🛏️ *Ambientes:* \${prop.ambientes} amb.\${prop.dormitorios ? ' | ' + prop.dormitorios + ' dorm.' : ''}\${prop.banos ? ' | ' + prop.banos + ' baño(s)' : ''}\n\` : ''}\${prop.financiacion?.length ? \`💳 *Financiación:* \${prop.financiacion.join(', ')}\n\` : ''}🔗 *Ver en web:* \${prop.url}

2. Si es ALQUILER, agregá los requisitos:
📋 *Requisitos para alquilar:*
• 1 garantía propietaria o 3 garantes con recibos de sueldo a conformidad del locador
• Justificación de ingresos del inquilino
• Gastos de ingreso: valor del alquiler x 4

3. Si es VENTA, no pongas requisitos.

4. Preguntá: *"¿Esta propiedad se ajusta a lo que estás buscando? ¿Te gustaría coordinar una visita o tenés alguna duda? 😊"*
...
6. Si confirmó que agendó, cerrá con la web: fracchiapropiedades.com.ar

7. Si pregunta algo que no sabés, respondé:
*"Disculpá, no tengo ese dato. Para consultarlo podés escribirle directamente a nuestro equipo: wa.me/\${env.WHATSAPP_CONSULTAS} 😊"*

Siempre cerrá tu respuesta con el bloque de triggers:
<triggers>
{
  "fichaEnviada": false,
  "linkEnviado": false,
